...
- **Fix 1 — writeConfig (app.js):** `writeConfig()` usaba `echo '${b64}' | base64 -d > file` que dependía del binario `base64` (no siempre presente en Android sh) y de pipes en KernelSU exec. Reemplazado por `printf '%s\n'` con escape de comillas simples — solo builtins POSIX. Esto resuelve "Saved Failed" / "Saved Failed Check Permission" en todos los botones Apply Changes.
