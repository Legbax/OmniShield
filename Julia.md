...
- **Fix 1a — writeConfig (app.js):** `writeConfig()` usaba `echo '${b64}' | base64 -d > file` que dependía del binario `base64` (no garantizado en BusyBox ash de KernelSU). Primer intento con `printf '%s\n'` seguía fallando porque `.join('\n')` introduce newlines literales en el command string — KernelSU `exec` no maneja multi-line commands. Fix definitivo: chained `echo` commands en un grupo `{ echo 'k=v'; echo 'k2=v2'; } > file` — single-line, solo builtins POSIX, sin newlines embebidos.
