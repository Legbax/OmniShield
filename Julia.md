...
- **Fix 1a — writeConfig (app.js):** `writeConfig()` usaba `echo '${b64}' | base64 -d > file` que dependía del binario `base64` (no garantizado en BusyBox ash de KernelSU). Segundo intento con `.join('\n')` + printf fallaba por newlines literales en el command string. Tercer intento con `{ echo 'k=v'; echo 'k2=v2'; } > file` fallaba por `{ }` grouping no soportado o largo del comando excediendo buffer de KernelSU exec. Fix definitivo: `printf '%s\\n' 'k=v' 'k2=v2' > file` — POSIX printf con múltiples argumentos (el format `%s\\n` se reutiliza per-arg). Comando corto (~800 chars vs ~3000), single-line, sin `{ }`, sin pipes. chmod separado en su propio ksu_exec para que su fallo no afecte el resultado. Read-back verification como fallback si errno no es confiable.
- **Fix 1b — ksu_exec errno type (app.js):** `r.errno || 0` no normalizaba el tipo. Si KernelSU devuelve errno como string `"0"`, entonces `"0" || 0` retorna `"0"` (truthy string) y `"0" === 0` es `false` (strict equality). Resultado: `writeConfig` siempre retornaba `false` sin importar si el comando tuvo éxito. Fix: `+r.errno || 0` — unary `+` coerce a number antes del OR. Afectaba también a Force Stop y Wipe (líneas 750, 758).
...
**Nota personal para el siguiente agente:** El scope ahora es 100% dinámico — controlado exclusivamente por `scoped_apps` en `.identity.cfg`. Si `scoped_apps` está vacío o no existe, NINGUNA app será hookeada (comportamiento correcto — antes se hookeaban 9 apps hardcodeadas incluso sin config). El SSAID es ahora determinístico por package name via `cksum` XOR `master_seed`. `cksum` es POSIX y está disponible en todos los Android (parte de toybox). Si se necesita cambiar el algoritmo de hash en el futuro, asegurar que sea compatible con mksh y no dependa de `md5sum`/`sha256sum` que pueden no estar disponibles. REGLA PARA ksu_exec: NUNCA pasar comandos con newlines literales al exec de KernelSU — usar chained commands con `;` en una sola línea. NUNCA usar `===` para comparar errno — usar `+r.errno` para coercion numérica primero. KernelSU puede devolver errno como string.
