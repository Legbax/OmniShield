...
**Fecha y agente:** 28 de febrero de 2026, Claude (PR70c — Memory mapping stealth + UX scope picker + scope save feedback)
**Resumen de cambios:** v12.9.50 — Ocultación de memory mappings del módulo, UX mejorada en scope picker, feedback de save en scope.
...
**Prompt del usuario:** "He seleccionado esta app en el scope en settings y no parece afectar el spoof. Esos memory mappings están siendo detectados."
...
* Entrada PR65: "SELinux Bypass Bridge implementado. Se añadió lógica en service.sh y app.js para copiar automáticamente .identity.cfg al directorio oficial del módulo (/data/adb/modules/omnishield/identity.cfg). Esto permite que Zygisk lea la configuración sin bloqueos de SELinux."
* Entrada PR67: "Fix crasheo app objetivo (SEGV_ACCERR). Se eliminó remapModuleMemory() ya que violaba las políticas W^X de SELinux al intentar asignar PROT_EXEC a memoria anónima. La ocultación de mapas recae exclusivamente en el VFS Hook (my_openat)."
