...
const char* my_eglQueryString(void* display, int name) {
    if (G_DEVICE_PROFILES.count(g_currentProfileName)) {
        ...
        if (name == EGL_EXTENSIONS_ENUM) {
            ...
            bool profileIsAdreno = (egl == "adreno");
...
std::string generateMulticoreCpuInfo(const DeviceFingerprint& fp) {
    ...
    if (platform.find("mt6768") != std::string::npos ||
        ...
        if (bigCoreStart < 0) bigCoreStart = 0;
...
int my_system_property_get(const char *key, char *value) {
    ...
    if (G_DEVICE_PROFILES.count(g_currentProfileName)) {
        ...
        else if (k == "ro.product.system.model" || k == "ro.product.vendor.model" || k == "ro.product.odm.model") dynamic_buffer = fp.model;
...
off_t my_lseek(int fd, off_t offset, int whence) {
    ...
    if (g_fdContentCache.count(fd)) {
        ...
        if (whence == SEEK_SET) pos = std::min((size_t)std::max((off_t)0, offset), size);
        else if (whence == SEEK_CUR) pos = std::min((size_t)std::max((off_t)0, (off_t)pos + offset), size);
        else if (whence == SEEK_END) pos = size;
        return (off_t)pos;
    }
...
// -----------------------------------------------------------------------------
static jstring my_SettingsSecure_getString_2args(JNIEnv* env, jstring name) {
...
static jstring my_SettingsSecure_getStringForUser_3args(JNIEnv* env, jstring name, jint userHandle) {
    return my_SettingsSecure_getString_2args(env, name);
}
...
public:
    ...
    void postAppSpecialize(zygisk::Api *api, JNIEnv *env) override {
        ...
        // Settings.Secure (Android ID) API < 30
        ...
        if (settings_func) DobbyHook(settings_func, (void*)my_SettingsSecure_getString_2args, (void**)&orig_SettingsSecure_getString_2args);
        ...
        if (settings_user_func) DobbyHook(settings_user_func, (void*)my_SettingsSecure_getStringForUser_3args, (void**)&orig_SettingsSecure_getStringForUser_3args);
