...
static bool isAshmemFd(int fd, std::string* outPath = nullptr) {
    if (fd < 0 || !orig_readlinkat) return false;
    char fdlink[64];
    char target[512] = {};
    snprintf(fdlink, sizeof(fdlink), "/proc/self/fd/%d", fd);
    ssize_t n = orig_readlinkat(AT_FDCWD, fdlink, target, sizeof(target) - 1);
    if (n <= 0) return false;
    target[n] = '\0';
    std::string t(target);
...
static void maybeLogAshmemFd(int fd, const char* from) {
    ...
    std::string path;
    (void)isAshmemFd(fd, &path); // solo para enriquecer log, ya clasificado como ashmem.
    {
        std::lock_guard<std::mutex> lock(g_fdMutex);
