...
read_config() {
    ...
    SCOPED_APPS=$(grep "^scoped_apps=" "$CONFIG" | cut -d'=' -f2-)
...
setup_tun() {
    ...
    ip addr add "${TUN_ADDR}/30" dev "$TUN_NAME" 2>/dev/null
    ip link set "$TUN_NAME" up
    ip route add default dev "$TUN_NAME" table "$ROUTE_TABLE" 2>/dev/null
    log "TUN interface $TUN_NAME created (${TUN_ADDR}/30)"
}
...
setup_iptables() {
    ...
    # Exclude proxy server itself from being redirected (avoid routing loop)
    iptables -t mangle -I "$CHAIN_MARK" -d "$PROXY_HOST" -j RETURN
    ...
    log "iptables rules applied (mark=$FWMARK, dns=$PROXY_DNS)"
}

# ─── Cleanup: remove all OMNI rules (idempotent) ──────────────────────
cleanup_iptables() {
    # Unhook from OUTPUT chains (suppress errors — may not exist)
    iptables -t mangle -D OUTPUT -j "$CHAIN_MARK" 2>/dev/null
    iptables -t nat -D OUTPUT -j "$CHAIN_DNS" 2>/dev/null
    ip6tables -D OUTPUT -j "$CHAIN_V6" 2>/dev/null
...
do_start() {
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        log "ERROR: Daemon failed to create PID file — check $LOG_FILE"
        cleanup_tun
        return 1
    fi
