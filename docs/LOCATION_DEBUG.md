...
7. **PR120 Ashmem tracer** logs when scoped processes use `/dev/ashmem` file descriptors via `open/read/pread/mmap`.
   - PR120 uses FD classification cache (ashmem/non-ashmem) to avoid repeated `readlinkat` in hot `read()` paths.
